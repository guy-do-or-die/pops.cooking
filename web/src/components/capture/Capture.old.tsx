...
export const Capture: React.FC<CaptureProps> = ({ disabled, popAddress: popAddressProp }) => {
    ...
    const [flowPhase, setFlowPhase] = useState<'ready' | 'preparing' | 'capturing' | 'verifying' | 'verified' | 'failed'>('ready');
    ...
    const generateChallengeOnPop = async (): Promise<string | null> => {
        if (!publicClient) {
    ...
    // Main flow: Generate challenge and auto-capture
    const snapPhoto = async () => {
        ...
        // Generate challenge (on-chain if wallet connected, local otherwise)
        if (wallets.length > 0) {
            // Generate on-chain with wallet signing and wait for receipt
            const newHash = await generateChallengeOnPop();
            
            if (!newHash) {
                // Challenge generation failed, reset
                setFlowPhase('ready');
                return;
            }
        ...
        // Start recording automatically after challenge is confirmed and state updated
        setFlowPhase('capturing');
        startRecording();
    };

    // Auto-verify when recording completes
    useEffect(() => {
        if (recordedBlob && flowPhase === 'capturing') {
            setFlowPhase('verifying');
            verifyRecording();
        }
    }, [recordedBlob]);

    // Update phase when verification completes
    useEffect(() => {
        if (verificationResult) {
            if (verificationResult.verified) {
                setFlowPhase('verified');
            } else {
                setFlowPhase('failed');
            }
        }
    ...
    const recordProgressOnChain = async () => {
        ...
        try {
            ...
            setTxStatus('✅ Progress sealed on-chain!');
    ...
    return (
        <div className="flex flex-col items-center gap-4 p-4 w-full max-w-4xl">
            ...
            <div className="flex gap-2 w-full max-w-[640px]">
                {flowPhase === 'verified' || flowPhase === 'failed' ? (
                    flowPhase === 'verified' ? (
                        <Button
